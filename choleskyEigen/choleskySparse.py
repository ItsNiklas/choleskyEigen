...
def register():
    # Register module
    xla.backend_specific_translations["cpu"][choleskySparse_p] = choleskySparse_xla_translation
...
def choleskySparse_impl(A_sp_data, A_sp_idx, A_sp_shape):
    raise NotImplementedError(f"Please JIT this function.")

# prim
def choleskySparse_prim(A_sp_data, A_sp_idx, A_sp):
    return choleskySparse_p.bind(A_sp_data, A_sp_idx, A_sp.todense()) #todense() cheat :(
...
def choleskySparse_xla_translation(c, A_sp_data, A_sp_idx, A):
    ...
    out_shape = xla_client.Shape.array_shape(jnp.dtype(dtype), c.get_shape(A).dimensions(), (0,1))
